...
let prizes;
const DEFAULT_MESS = [
  "¿Debería ganar el primer premio o el primer premio, enredado...",
  "¡Escuché que tienes que ser vegetariano con un mes de anticipación para ganar el gran premio!",
  "¡¡¡Realmente quiero el primer premio!!!",
  "¿Alguien quiere el primer premio?",
  "El quinto premio también es bueno, siempre que puedas ganarlo",
  "¡Les deseo a todos un Feliz Año Nuevo!",
  "Ganar la lotería no es importante, todos comen y beben bien",
  "Año nuevo, te deseo lo mejor en todo",
  "Como escort profesional, voy a ver si alguna de vosotras es como yo",
  "¡Bendice el nuevo año a todos cada vez mejor!",
  "¡¡Lucha de nuevo el año que viene!!!"
];
...
function showPrizeList(currentPrizeIndex) {
  ...
  if (currentPrize.type === defaultType) {
    currentPrize.count === "Sin limite";
  }
  let htmlCode = `<div class="prize-mess">Extrayendo <label id="prizeType" class="prize-shine">${currentPrize.text}</label><label id="prizeText" class="prize-shine">${currentPrize.title}</label>， restantes <label id="prizeLeft" class="prize-shine">${currentPrize.count}</label>个</div><ul class="prize-list">`;
  prizes.forEach(item => {
...
let setPrizeData = (function () {
  return function (currentPrizeIndex, count, isInit) {
    ...
    if (currentPrizeIndex === 0) {
      prizeElement.prizeType.textContent = "Premio Especial";
      prizeElement.prizeText.textContent = " ";
      prizeElement.prizeLeft.textContent = "Sin límite";
      return;
    }
